...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        site_df = spacex_df[spacex_df['Launch Site']== entered_site]['class'].value_counts().to_frame()
        site_df.reset_index(inplace=True)
        site_df.columns = ['class', 'count']
        fig = px.pie(site_df, values='count', 
        names='class', 
        title='Total Successful Launches of Site:{}'.format(entered_site))
        return fig
...
def payload_scatter(entered_site, entered_payload):
    if entered_site == 'ALL':
